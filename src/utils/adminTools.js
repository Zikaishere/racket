...
async function handleSetField(guildId, actorId, target, field, amount) {
  ...
  await logAudit({ guildId, actorId, targetId: target.id, action: `admin_set_${field}`, amount: safeAmount, currency: field });
...
function buildAdminOverviewEmbed(prefix) {
  return embed.raw(0x2b2d31)
    ...
    .addFields(
      { name: 'Available Commands', value: ADMIN_OVERVIEW_COMMANDS.map((line) => `\`${prefix}${line}\``).join('\n'), inline: false },
