...
const run = async ({ userId, guildId, targetUser, reply }) => {
  ...
  if (remaining > 0) {
    ...
    return reply({ embeds: [embed.warning('Lay Low', `The cops are still looking for you. You can rob again in **${hours}h ${mins}m**.`)] });
  }

  const victim = await getUser(targetUser.id, guildId);
  if (victim.balance < ROB_MIN_BAL) {
    return reply({ embeds: [embed.error(`${targetUser.username} is too poor to rob. They need at least ${fmt(ROB_MIN_BAL)}.`)] });
  }
  ...
  if (success) {
    ...
    await logAudit({ guildId, actorId: userId, targetId: targetUser.id, action: 'robbery_success', amount: stolen, currency: 'wallet' });
    return reply({ embeds: [embed.success('Robbery Successful', `You managed to steal ${fmt(stolen)} from **${targetUser.username}**.`)] });
  }
  ...
  await logAudit({ guildId, actorId: userId, targetId: targetUser.id, action: 'robbery_failed', amount: actualFine, currency: 'wallet' });
  const message = actualFine > 0
...
module.exports = {
  ...
  async execute({ message }) {
    ...
    return run({ userId: message.author.id, guildId: message.guild.id, targetUser: target, reply: data => message.reply(data) });
  },

  async executeSlash({ interaction }) {
    const target = interaction.options.getUser('user');
    return run({ userId: interaction.user.id, guildId: interaction.guild.id, targetUser: target, reply: data => interaction.reply(data) });
  },
};