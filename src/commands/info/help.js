...
module.exports = {
  ...
  slash: new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('command').setDescription('Specific command or category to look up').setRequired(false)),
  ...
  async execute({ message, args, client, prefix }) {
      ...
      if (cmd && !(cmd.category === 'dev' && !DEV_IDS.includes(message.author.id))) {
        ...
        const e = embed.info(`${icon} ${cmd.name}`, cmd.description)
          .addFields(
            ...
            { name: 'Aliases', value: cmd.aliases?.length ? cmd.aliases.map((alias) => `\`${alias}\``).join(', ') : 'None', inline: true },
          );
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (selected === 'overview') {
        ...
        await interaction.update({ embeds: [buildCategoryEmbed(client, selected, prefix)], components: [buildSelectMenu(client, selected)] });
      }
    });
  ...
  async executeSlash({ interaction, client }) {
    ...
    collector.on('collect', async (menuInteraction) => {
      ...
      if (selected === 'overview') {
        await menuInteraction.update({ embeds: [buildMainEmbed(client, prefix)], components: [buildSelectMenu(client)] });
      } else {
        await menuInteraction.update({ embeds: [buildCategoryEmbed(client, selected, prefix)], components: [buildSelectMenu(client, selected)] });
      }
    });
    collector.on('end', () => msg.edit({ components: [] }).catch(() => {}));
  },
