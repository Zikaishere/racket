...
const ALARM_CHANCE = [0.10, 0.20, 0.35, 0.50, 0.65, 0.85];

function buildButtons(takeValue, disabled = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('vault_crack').setLabel('Crack Lock').setStyle(ButtonStyle.Primary).setDisabled(disabled),
    new ButtonBuilder().setCustomId('vault_take').setLabel(`Take ${takeValue.toLocaleString()} chips`).setStyle(ButtonStyle.Success).setDisabled(disabled)
  );
}
...
const handleCrack = async (interaction) => {
  ...
  if (level >= MULTIPLIERS.length) {
    return interaction.reply({ embeds: [embed.error('You already beat the highest level. Take the money!')], ephemeral: true });
  }
  ...
  if (tripped) {
    ...
    const bustedEmbed = embed.raw(0xFF6B6B)
      .setTitle('Busted')
      .setDescription(`The alarm went off on Door ${level + 1}.\n\nTotal Lost: ${game.originalBet.toLocaleString()} chips`);
  ...
  if (game.level >= MULTIPLIERS.length) {
    const finalEmbed = embed.raw(0xFFB703)
      .setTitle('Vault Cracked')
      .setDescription(`You disabled the final lock.\n\nPool: ${game.pool.toLocaleString()} chips\n\nThere are no more doors. Take the money and run.`);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('vault_take').setLabel(`Take ${game.pool.toLocaleString()} chips`).setStyle(ButtonStyle.Success)
    );
...
const handleTake = async (interaction) => {
  ...
  if (game.level >= 4) {
    CasinoManager.addHighlight(game.guildId, `**${game.username}** escaped a level ${game.level} vault with ${game.pool.toLocaleString()} chips.`);
  }

  activeGames.delete(userId);

  const safeEmbed = embed.raw(0x2DC653)
    .setTitle('Safe Escape')
    .setDescription(`You got out clean.\n\nYou walked away with ${game.pool.toLocaleString()} chips.\nNew Chips: ${user.chips.toLocaleString()}`);
