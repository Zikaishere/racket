...
function buildButtons(takeValue, disabled = false) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('double_flip').setLabel('Double Again').setStyle(ButtonStyle.Danger).setDisabled(disabled),
    new ButtonBuilder().setCustomId('double_take').setLabel(`Take ${takeValue.toLocaleString()} chips`).setStyle(ButtonStyle.Success).setDisabled(disabled)
  );
}
...
const run = async ({ userId, guildId, username, bet, reply }) => {
  ...
  if (activeGames.has(userId)) {
    return reply({ embeds: [embed.error('You already have an active Double or Nothing game running!')], ephemeral: true });
  }
...
const handleFlip = async (userId, interaction, replyFn) => {
  ...
  const loseEmbed = embed.raw(0xFF6B6B)
    .setTitle('Double or Nothing')
    .setDescription(`The coin landed on Tails. You lost the pool.\n\nTotal Lost: ${game.originalBet.toLocaleString()} chips`)
...
const handleTake = async (interaction) => {
  ...
  if (game.pool >= game.originalBet * 8) {
    CasinoManager.addHighlight(game.guildId, `**${game.username}** chained ${game.flips} flips in Double or Nothing and cashed out ${game.pool.toLocaleString()} chips.`);
  }

  activeGames.delete(userId);

  const takeEmbed = embed.raw(0x2DC653)
    .setTitle('Double or Nothing')
    .setDescription(`You cashed out safely.\n\nYou walked away with ${game.pool.toLocaleString()} chips.\nNew Chips: ${user.chips.toLocaleString()}`)
