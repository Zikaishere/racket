...
const run = async ({ userId, guildId, action, rawAmount, evade, reply }) => {
  ...
  if (action === 'buy') {
    ...
    if (isNaN(amount) || amount <= 0) return reply({ embeds: [embed.error('Please specify a valid amount.')], ephemeral: true });
    if (user.balance < amount) return reply({ embeds: [embed.error(`You don't have enough raqs.\n\nWallet: ${fmt(user.balance)}`)], ephemeral: true });
    ...
    return reply({ embeds: [embed.success('Chips Purchased', `You bought **${amount.toLocaleString()}** chips.\n\nWallet: ${fmt(user.balance)}\nChips: **${user.chips.toLocaleString()}**`)] });
  }
  ...
  if (action === 'cashout') {
    ...
    if (isNaN(amount) || amount <= 0) return reply({ embeds: [embed.error('Please specify a valid amount.')], ephemeral: true });
    if (user.chips < amount) return reply({ embeds: [embed.error(`You don't have enough chips.\n\nChips: **${user.chips.toLocaleString()}**`)], ephemeral: true });
    ...
    if (amount >= TAX_THRESHOLD) {
      if (evade) {
        ...
        if (roll < 0.3) {
          ...
          taxAmount = Math.floor(amount * 0.10);
          ...
          await user.save();
          await logAudit({ guildId, actorId: userId, targetId: userId, action: 'cashier_cashout_evade_busted', amount: fineAmount, currency: 'wallet' });
          return reply({ embeds: [embed.error(`You were caught attempting to evade taxes on your cashout. They seized ${fmt(fineAmount)} and barred you from the casino for 1 hour.`)] });
        }
      } else {
    ...
    await user.save();
    await logAudit({ guildId, actorId: userId, targetId: userId, action: auditAction, amount: finalAmount, currency: 'wallet', metadata: { originalAmount: amount, taxAmount, evade } });

    return reply({ embeds: [embed.success('Chips Cashed Out', `You cashed out **${amount.toLocaleString()}** chips for **${fmt(finalAmount)}**.${taxMessage}\n\nWallet: ${fmt(user.balance)}\nChips: **${user.chips.toLocaleString()}**`)] });
  }
};
...
module.exports = {
  ...
  slash: new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('buy').setDescription('Buy chips with raqs')
      .addStringOption(o => o.setName('amount').setDescription('Amount or "all"').setRequired(true)))
    .addSubcommand(s => s.setName('cashout').setDescription('Cash out chips for raqs')
      .addStringOption(o => o.setName('amount').setDescription('Amount or "all"').setRequired(true))
      .addBooleanOption(o => o.setName('evade').setDescription('Attempt to evade the Syndicate tax').setRequired(false))),
  ...
  async execute({ message, args }) {
    ...
    return run({ userId: message.author.id, guildId: message.guild.id, action, rawAmount, evade, reply: data => message.reply(data) });
  },
