...
const run = async ({ userId, guildId, query, bet, reply }) => {
  ...
  if (!chicken) {
    return reply({ embeds: [embed.error('That chicken was not found in your inventory. Use `inventory` to get the slot number.')] });
  }
  ...
  return reply({
    embeds: [
      embed.raw(won ? 0x2DC653 : 0xFF6B6B)
        .setTitle('Cockfight')
...
module.exports = {
  ...
  slash: new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName('chicken').setDescription('Inventory slot number or chicken name').setRequired(true))
  ...
  async execute({ message, args }) {
    ...
    return run({ userId: message.author.id, guildId: message.guild.id, query, bet, reply: data => message.reply(data) });
  },
