...
module.exports = {
  ...
  async execute({ message, args }) {
    ...
    if (!newPrefix || newPrefix.length > 5) {
      return message.reply({ embeds: [embed.error('Usage: `.setprefix <newPrefix>` and the prefix must be 1-5 characters.')] });
    }
  ...
  async executeSlash({ interaction }) {
    ...
    return interaction.reply({ embeds: [await setPrefix(interaction.guild.id, interaction.user.id, value)], ephemeral: true });
  },
};
